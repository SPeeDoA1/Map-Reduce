# PowerGrid AI - Distributed Energy Intelligence Platform

> **Enterprise-Scale MapReduce System for Grid Analytics**
> Built with Apache Spark (PySpark) | No ML Required | Transparent Algorithms

---

## Table of Contents

1. [Executive Summary](#1-executive-summary)
2. [System Architecture](#2-system-architecture)
3. [MapReduce Workflow](#3-mapreduce-workflow)
4. [Project Structure](#4-project-structure)
5. [Installation Guide](#5-installation-guide)
6. [Running the System](#6-running-the-system)
7. [Pipeline Deep Dive](#7-pipeline-deep-dive)
8. [Output & Results](#8-output--results)
9. [Configuration](#9-configuration)
10. [Troubleshooting](#10-troubleshooting)

---

## 1. Executive Summary

**PowerGrid AI** is a distributed analytics platform that simulates and optimizes a national energy grid using **Apache Spark's MapReduce paradigm**. Instead of relying on "black box" machine learning models, this system uses **transparent, deterministic MapReduce algorithms** to process massive datasets.

### What It Does

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         POWERGRID AI CAPABILITIES                           │
├─────────────────────────────────────────────────────────────────────────────┤
│  1. SIMULATE     → Generate realistic grid data (meters, plants, weather)   │
│  2. AGGREGATE    → Sum energy demand by district using MapReduce            │
│  3. OPTIMIZE     → Calculate weather-adjusted power supply                  │
│  4. BALANCE      → Join supply & demand to find blackout risks              │
│  5. DETECT       → Flag fraudulent meters using Z-score statistics          │
│  6. TRACK        → Monitor carbon emissions by plant type                   │
│  7. VISUALIZE    → Dashboard + charts for business intelligence             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Key Metrics

| Metric | Value |
|--------|-------|
| Smart Meters | ~5,000 simulated |
| Power Plants | 50 (nuclear, coal, gas, hydro, wind, solar) |
| Districts | 10 geographic regions |
| Data Points | ~300,000 readings |
| Processing | < 2 minutes on local machine |

---

## 2. System Architecture

### High-Level Overview

```
╔═══════════════════════════════════════════════════════════════════════════════╗
║                              DATA FLOW ARCHITECTURE                            ║
╠═══════════════════════════════════════════════════════════════════════════════╣
║                                                                               ║
║  ┌─────────────────────────────────────────────────────────────────────────┐  ║
║  │                         DATA GENERATION LAYER                           │  ║
║  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                   │  ║
║  │  │Smart Meters  │  │ Power Plants │  │   Weather    │                   │  ║
║  │  │  Simulator   │  │  Simulator   │  │  Simulator   │                   │  ║
║  │  │   ~5,000     │  │     50       │  │   72 hours   │                   │  ║
║  │  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘                   │  ║
║  └─────────┼─────────────────┼─────────────────┼───────────────────────────┘  ║
║            │                 │                 │                              ║
║            ▼                 ▼                 ▼                              ║
║  ┌─────────────────────────────────────────────────────────────────────────┐  ║
║  │                              RAW DATA (CSV)                             │  ║
║  │  meter_readings.csv  │  plant_output.csv  │  weather.csv  │  *.csv      │  ║
║  └─────────────────────────────────────────────────────────────────────────┘  ║
║                                      │                                        ║
║                                      ▼                                        ║
║  ┌─────────────────────────────────────────────────────────────────────────┐  ║
║  │                     APACHE SPARK PROCESSING LAYER                       │  ║
║  │                                                                         │  ║
║  │   ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐   │  ║
║  │   │ Job 1   │   │ Job 2   │   │ Job 3   │   │ Job 4   │   │ Job 5   │   │  ║
║  │   │ Demand  │──▶│ Supply  │──▶│  Grid   │   │  Fraud  │   │ Carbon  │   │  ║
║  │   │Aggreg.  │   │ Optim.  │   │ Balance │   │ Detect  │   │ Track   │   │  ║
║  │   └─────────┘   └─────────┘   └────┬────┘   └─────────┘   └─────────┘   │  ║
║  │        │             │             │             │             │        │  ║
║  │        └─────────────┴──────┬──────┴─────────────┴─────────────┘        │  ║
║  └─────────────────────────────┼───────────────────────────────────────────┘  ║
║                                ▼                                              ║
║  ┌─────────────────────────────────────────────────────────────────────────┐  ║
║  │                         PRESENTATION LAYER                              │  ║
║  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                   │  ║
║  │  │   Console    │  │  Matplotlib  │  │  CSV/JSON    │                   │  ║
║  │  │  Dashboard   │  │   Charts     │  │   Reports    │                   │  ║
║  │  └──────────────┘  └──────────────┘  └──────────────┘                   │  ║
║  └─────────────────────────────────────────────────────────────────────────┘  ║
║                                                                               ║
╚═══════════════════════════════════════════════════════════════════════════════╝
```

### Technology Stack

```
┌────────────────────────────────────────────────────────────────┐
│                      TECHNOLOGY STACK                          │
├────────────────────────────────────────────────────────────────┤
│  Language        │  Python 3.8+                                │
│  Big Data        │  Apache Spark 3.x (PySpark)                 │
│  Data Processing │  Pandas, NumPy                              │
│  Data Generation │  Faker                                      │
│  Visualization   │  Matplotlib, Seaborn                        │
│  Console UI      │  Rich (optional)                            │
│  Testing         │  Pytest                                     │
│  Runtime         │  Java 11 or 17 (required for Spark)         │
└────────────────────────────────────────────────────────────────┘
```

---

## 3. MapReduce Workflow

### What is MapReduce?

MapReduce is a programming model for processing large datasets in parallel:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         MAPREDUCE PARADIGM                                  │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   INPUT DATA                MAP PHASE                    REDUCE PHASE       │
│   ──────────               ──────────                   ─────────────       │
│                                                                             │
│   [Record 1] ──────▶  (key1, value1)  ──┐                                   │
│   [Record 2] ──────▶  (key1, value2)  ──┼──▶  (key1, [v1,v2,v3]) ──▶ SUM    │
│   [Record 3] ──────▶  (key1, value3)  ──┘                                   │
│                                                                             │
│   [Record 4] ──────▶  (key2, value4)  ──┐                                   │
│   [Record 5] ──────▶  (key2, value5)  ──┼──▶  (key2, [v4,v5])    ──▶ SUM    │
│                                         │                                   │
│                                                                             │
│   "Transform each record"      "Group by key"      "Aggregate values"       │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### The 5 MapReduce Pipelines

#### Pipeline 1: Demand Aggregation

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  PIPELINE 1: DEMAND AGGREGATION                                             │
│  Goal: Calculate total energy consumption per district                      │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  INPUT: meter_readings.csv                                                  │
│  ┌────────────────────────────────────────────────────────┐                 │
│  │ timestamp, meter_id, district_id, meter_type, kwh, ... │                 │
│  └────────────────────────────────────────────────────────┘                 │
│                              │                                              │
│                              ▼                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  MAP: (meter_reading) ──▶ (district_id, kwh)                        │    │
│  │                                                                     │    │
│  │  Example:                                                           │    │
│  │    {district: "D001", kwh: 5.2} ──▶ ("D001", 5.2)                   │    │
│  │    {district: "D001", kwh: 3.1} ──▶ ("D001", 3.1)                   │    │
│  │    {district: "D002", kwh: 8.7} ──▶ ("D002", 8.7)                   │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                              │                                              │
│                              ▼                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  REDUCE: (district_id, [kwh_list]) ──▶ (district_id, total_kwh)     │    │
│  │                                                                     │    │
│  │  Example:                                                           │    │
│  │    ("D001", [5.2, 3.1, 2.4, ...]) ──▶ ("D001", 45892.5)             │    │
│  │    ("D002", [8.7, 4.2, 6.1, ...]) ──▶ ("D002", 62341.2)             │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                              │                                              │
│                              ▼                                              │
│  OUTPUT: Total demand per district                                          │
│  ┌────────────────────────────────────┐                                     │
│  │ district_id │ total_demand_kwh     │                                     │
│  │ D001        │ 45,892.5             │                                     │
│  │ D002        │ 62,341.2             │                                     │
│  └────────────────────────────────────┘                                     │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### Pipeline 2: Supply Optimization

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  PIPELINE 2: SUPPLY OPTIMIZATION                                            │
│  Goal: Calculate weather-adjusted power generation per district             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  INPUT: plant_output.csv + weather.csv                                      │
│                                                                             │
│  MAP:  (plant_log) ──▶ (district_id, output_mw)                             │
│                                                                             │
│        Weather adjustments applied:                                         │
│        • Solar: output × cloud_factor (0.0 - 1.0)                           │
│        • Wind:  output × wind_factor  (0.0 - 1.0)                           │
│        • Hydro: output × precipitation_factor                               │
│                                                                             │
│  REDUCE: (district_id, [mw_list]) ──▶ (district_id, total_supply_mw)        │
│                                                                             │
│  OUTPUT: Total supply per district (weather-adjusted)                       │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### Pipeline 3: Grid Balancing (JOIN Operation)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  PIPELINE 3: GRID BALANCING                                                 │
│  Goal: Identify districts at risk of blackout                               │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  This pipeline demonstrates the JOIN operation in MapReduce                 │
│                                                                             │
│    DEMAND_RDD                    SUPPLY_RDD                                 │
│    ──────────                    ──────────                                 │
│    (D001, 45892)                 (D001, 50000)                              │
│    (D002, 62341)                 (D002, 45000)   ◄── Deficit!               │
│    (D003, 38291)                 (D003, 40000)                              │
│         │                             │                                     │
│         └──────────────┬──────────────┘                                     │
│                        │                                                    │
│                        ▼                                                    │
│               ┌─────────────────┐                                           │
│               │      JOIN       │                                           │
│               │  on district_id │                                           │
│               └────────┬────────┘                                           │
│                        │                                                    │
│                        ▼                                                    │
│         (district_id, (demand, supply))                                     │
│                        │                                                    │
│                        ▼                                                    │
│    ┌───────────────────────────────────────────────────────────────┐        │
│    │  CALCULATE: net_load = supply - demand                        │        │
│    │  FILTER:    net_load < 0  ──▶  BLACKOUT RISK                  │        │
│    └───────────────────────────────────────────────────────────────┘        │
│                        │                                                    │
│                        ▼                                                    │
│    ┌─────────────────────────────────────────────────────────────────┐      │
│    │ district │ demand  │ supply  │ net_load │ status               │      │
│    │ D001     │ 45,892  │ 50,000  │ +4,108   │ NORMAL               │      │
│    │ D002     │ 62,341  │ 45,000  │ -17,341  │ CRITICAL ⚠️           │      │
│    │ D003     │ 38,291  │ 40,000  │ +1,709   │ NORMAL               │      │
│    └─────────────────────────────────────────────────────────────────┘      │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### Pipeline 4: Fraud Detection (Statistical Analysis)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  PIPELINE 4: FRAUD DETECTION                                                │
│  Goal: Detect energy theft using Z-score statistical analysis               │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  TWO-PASS MAPREDUCE ALGORITHM                                               │
│                                                                             │
│  ╔═══════════════════════════════════════════════════════════════════════╗  │
│  ║  PASS 1: Calculate Statistics per District                            ║  │
│  ╠═══════════════════════════════════════════════════════════════════════╣  │
│  ║                                                                       ║  │
│  ║  MAP:    (meter) ──▶ (district_id, (kwh, kwh², 1))                    ║  │
│  ║                                                                       ║  │
│  ║  REDUCE: (district_id, values) ──▶ (district_id, sum, sum_sq, count)  ║  │
│  ║                                                                       ║  │
│  ║  COMPUTE: mean = sum / count                                          ║  │
│  ║           variance = (sum_sq / count) - mean²                         ║  │
│  ║           stddev = √variance                                          ║  │
│  ╚═══════════════════════════════════════════════════════════════════════╝  │
│                              │                                              │
│                              ▼                                              │
│  ╔═══════════════════════════════════════════════════════════════════════╗  │
│  ║  PASS 2: Calculate Z-Scores                                           ║  │
│  ╠═══════════════════════════════════════════════════════════════════════╣  │
│  ║                                                                       ║  │
│  ║  JOIN: meter_data with district_statistics                            ║  │
│  ║                                                                       ║  │
│  ║  FORMULA:  Z = (meter_usage - district_mean) / district_stddev        ║  │
│  ║                                                                       ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────┐  ║  │
│  ║  │  Z-Score Interpretation:                                        │  ║  │
│  ║  │                                                                 │  ║  │
│  ║  │    Z < -3.0  ──▶  FRAUD LIKELY (under-reporting/theft)          │  ║  │
│  ║  │   -3.0 to -2.5 ──▶  SUSPICIOUS                                  │  ║  │
│  ║  │   -2.5 to +2.5 ──▶  NORMAL                                      │  ║  │
│  ║  │    Z > +3.0  ──▶  ANOMALY (possible meter malfunction)          │  ║  │
│  ║  └─────────────────────────────────────────────────────────────────┘  ║  │
│  ╚═══════════════════════════════════════════════════════════════════════╝  │
│                                                                             │
│  VISUAL: Normal Distribution with Fraud Threshold                           │
│                                                                             │
│                            Normal Distribution                              │
│                                   ▲                                         │
│                                  ╱│╲                                        │
│                                 ╱ │ ╲                                       │
│                                ╱  │  ╲                                      │
│                               ╱   │   ╲                                     │
│                              ╱    │    ╲                                    │
│                             ╱     │     ╲                                   │
│                            ╱      │      ╲                                  │
│                           ╱       │       ╲                                 │
│                    ──────╱────────┼────────╲──────                          │
│                    ▲              │              ▲                          │
│               Z = -3.0      Mean (Z=0)      Z = +3.0                        │
│                 │                               │                           │
│                 │         99.7% Normal          │                           │
│                 │◄─────────────────────────────►│                           │
│                 │                               │                           │
│            FRAUD ZONE                      ANOMALY ZONE                     │
│           (theft likely)                (meter malfunction)                 │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### Pipeline 5: Carbon Tracking

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  PIPELINE 5: CARBON FOOTPRINT TRACKER                                       │
│  Goal: Monitor environmental impact by plant type                           │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  MAP:  (plant_output) ──▶ (plant_type, co2_tons)                            │
│                                                                             │
│        Emission Factors (tons CO2 per MWh):                                 │
│        ┌────────────┬──────────────┐                                        │
│        │ Plant Type │ CO2 Factor   │                                        │
│        ├────────────┼──────────────┤                                        │
│        │ Coal       │ 1.0          │  ████████████████████                  │
│        │ Gas        │ 0.4          │  ████████                              │
│        │ Nuclear    │ 0.0          │                                        │
│        │ Hydro      │ 0.0          │                                        │
│        │ Wind       │ 0.0          │                                        │
│        │ Solar      │ 0.0          │                                        │
│        └────────────┴──────────────┘                                        │
│                                                                             │
│  REDUCE: (plant_type, [co2_list]) ──▶ (plant_type, total_emissions)         │
│                                                                             │
│  ADDITIONAL METRICS:                                                        │
│  • Renewable percentage (target: 40%)                                       │
│  • Emission intensity (tons CO2 / MWh generated)                            │
│  • Carbon status: GREEN / OK / WARNING / CRITICAL                           │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Complete Execution Flow

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        COMPLETE EXECUTION FLOW                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   ./run.sh                                                                  │
│       │                                                                     │
│       ▼                                                                     │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │ PHASE 1: DATA GENERATION                                            │   │
│   │                                                                     │   │
│   │   [1] Weather Simulator ──▶ weather.csv (72 hours)                  │   │
│   │   [2] Smart Meter Simulator ──▶ meter_readings.csv (~300k records)  │   │
│   │   [3] Power Plant Simulator ──▶ plant_output.csv (3,600 records)    │   │
│   │                                                                     │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│       │                                                                     │
│       ▼                                                                     │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │ PHASE 2: SPARK INITIALIZATION                                       │   │
│   │                                                                     │   │
│   │   • Create SparkSession (local[*] = all CPU cores)                  │   │
│   │   • Configure memory (4GB driver, 4GB executor)                     │   │
│   │   • Set parallelism = 8 partitions                                  │   │
│   │                                                                     │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│       │                                                                     │
│       ▼                                                                     │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │ PHASE 3: MAPREDUCE ANALYTICS                                        │   │
│   │                                                                     │   │
│   │   JOB 1 ──▶ Demand Aggregation ──▶ total_demand/                    │   │
│   │       │                                                             │   │
│   │       ▼                                                             │   │
│   │   JOB 2 ──▶ Supply Optimization ──▶ total_supply/                   │   │
│   │       │                                                             │   │
│   │       ├─────────────────────────┐                                   │   │
│   │       ▼                         ▼                                   │   │
│   │   JOB 3 ──▶ Grid Balancing  JOB 5 ──▶ Carbon Tracking               │   │
│   │       │         │                         │                         │   │
│   │       │         ▼                         ▼                         │   │
│   │       │    grid_balance/            carbon/                         │   │
│   │       │                                                             │   │
│   │       ▼                                                             │   │
│   │   JOB 4 ──▶ Fraud Detection ──▶ fraud/                              │   │
│   │                                                                     │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│       │                                                                     │
│       ▼                                                                     │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │ PHASE 4: VISUALIZATION                                              │   │
│   │                                                                     │   │
│   │   • Console Dashboard (Rich library)                                │   │
│   │   • Generate 10 Matplotlib charts ──▶ data/reports/                 │   │
│   │                                                                     │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│       │                                                                     │
│       ▼                                                                     │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │ COMPLETE!                                                           │   │
│   │                                                                     │   │
│   │   Results available in:                                             │   │
│   │   • data/processed/ (CSV outputs from each job)                     │   │
│   │   • data/reports/   (PNG charts)                                    │   │
│   │                                                                     │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 4. Project Structure

```
PowerGridAI/
│
├── config/                          # Configuration files
│   ├── __init__.py
│   ├── spark_config.py              # Spark session settings
│   ├── grid_parameters.py           # Grid physics & economic constants
│   └── thresholds.py                # Alert thresholds & limits
│
├── data/
│   ├── raw/                         # Generated input data
│   │   ├── districts.csv            # District metadata
│   │   ├── meters.csv               # Meter metadata
│   │   ├── meter_readings.csv       # Smart meter readings (~300k rows)
│   │   ├── plants.csv               # Power plant metadata
│   │   ├── plant_output.csv         # Plant generation data
│   │   └── weather.csv              # Weather conditions
│   │
│   ├── processed/                   # MapReduce outputs
│   │   ├── demand/                  # Job 1 results
│   │   ├── supply/                  # Job 2 results
│   │   ├── balance/                 # Job 3 results
│   │   ├── fraud/                   # Job 4 results
│   │   └── carbon/                  # Job 5 results
│   │
│   └── reports/                     # Generated charts (PNG)
│       ├── demand_by_district.png
│       ├── demand_by_type.png
│       ├── supply_by_type.png
│       ├── capacity_utilization.png
│       ├── grid_balance.png
│       ├── fraud_zscore_dist.png
│       ├── fraud_status_dist.png
│       ├── emissions_by_type.png
│       ├── hourly_emissions.png
│       └── renewable_mix.png
│
├── src/
│   ├── __init__.py
│   │
│   ├── data_generation/             # Synthetic data generators
│   │   ├── __init__.py
│   │   ├── smart_meter_simulator.py # Generates meter readings
│   │   ├── power_plant_simulator.py # Generates plant output
│   │   └── weather_simulator.py     # Generates weather data
│   │
│   ├── spark_jobs/                  # MapReduce pipelines
│   │   ├── __init__.py
│   │   ├── demand_aggregator.py     # Pipeline 1: Demand aggregation
│   │   ├── supply_optimizer.py      # Pipeline 2: Supply optimization
│   │   ├── grid_balancer.py         # Pipeline 3: Grid balancing
│   │   ├── fraud_detector.py        # Pipeline 4: Fraud detection
│   │   └── carbon_tracker.py        # Pipeline 5: Carbon tracking
│   │
│   ├── visualization/               # Output & display
│   │   ├── __init__.py
│   │   ├── dashboard.py             # Console dashboard (Rich)
│   │   └── charts.py                # Matplotlib visualizations
│   │
│   └── main.py                      # Main orchestrator
│
├── tests/                           # Unit tests
│   ├── __init__.py
│   └── test_data_generation.py
│
├── requirements.txt                 # Python dependencies
├── run.sh                           # One-click execution script
└── README.md                        # This file
```

---

## 5. Installation Guide

### Prerequisites

| Requirement | Version | Check Command |
|-------------|---------|---------------|
| Python | 3.8+ | `python3 --version` |
| Java | 11 or 17 | `java -version` |
| pip | Latest | `pip --version` |

### Step 1: Install Java 17

**macOS (Homebrew):**
```bash
brew install openjdk@17
```

**Ubuntu/Debian:**
```bash
sudo apt update
sudo apt install openjdk-17-jdk
```

**Windows:**
Download from [Adoptium](https://adoptium.net/temurin/releases/?version=17)

### Step 2: Clone/Download the Project

```bash
cd /path/to/your/projects
# If cloning from git:
# git clone <repository-url>
cd PowerGridAI
```

### Step 3: Create Virtual Environment

```bash
# Create virtual environment
python3 -m venv venv

# Activate it
# macOS/Linux:
source venv/bin/activate
# Windows:
# venv\Scripts\activate

# Upgrade pip
pip install --upgrade pip
```

### Step 4: Install Dependencies

```bash
pip install -r requirements.txt
```

### Step 5: Verify Installation

```bash
# Check PySpark
python3 -c "import pyspark; print(f'PySpark {pyspark.__version__} installed')"

# Check other dependencies
python3 -c "import pandas, numpy, faker, matplotlib; print('All dependencies OK')"
```

---

## 6. Running the System

### Quick Start (Recommended)

```bash
# Make script executable (first time only)
chmod +x run.sh

# Run the full pipeline
./run.sh
```

### Command Line Options

```bash
# Generate new data and run analytics
./run.sh

# Force regenerate all data
./run.sh --regenerate

# Skip data generation (use existing data)
./run.sh --skip-data

# Skip chart generation
./run.sh --skip-charts

# Show help
./run.sh --help
```

### Manual Execution

```bash
# Activate virtual environment
source venv/bin/activate

# Set Java (if needed)
export JAVA_HOME=$(/usr/libexec/java_home -v 17)  # macOS
# export JAVA_HOME=/usr/lib/jvm/java-17-openjdk   # Linux

# Run
python -m src.main
```

### Expected Output

```
╔═══════════════════════════════════════════════════════════════════════════╗
║                                                                           ║
║   ⚡ PowerGrid AI - Distributed Energy Intelligence Platform              ║
║                                                                           ║
╚═══════════════════════════════════════════════════════════════════════════╝

======================================================================
PHASE 1: DATA GENERATION
======================================================================
[1/3] Generating weather data...
[2/3] Generating smart meter data...
[3/3] Generating power plant data...
✅ Data generation complete!

======================================================================
PHASE 2: MAPREDUCE ANALYTICS
======================================================================
JOB 1: DEMAND AGGREGATION
  MAP phase: (meter_log) -> (district_id, kwh)
  REDUCE phase: (district_id, [kwh_list]) -> (district_id, total_demand)
  ✅ Aggregated demand for 10 districts

JOB 2: SUPPLY OPTIMIZATION
  ...

JOB 3: GRID BALANCING
  ...

JOB 4: FRAUD DETECTION
  ...

JOB 5: CARBON TRACKING
  ...

======================================================================
PHASE 3: DASHBOARD
======================================================================
[Dashboard output with tables and metrics]

======================================================================
EXECUTION COMPLETE
======================================================================
📁 Data directory: data/
📊 Reports directory: data/reports/
⏱️  Completed at: 2024-06-15 14:32:45
```

---

## 7. Pipeline Deep Dive

### Data Generation Details

#### Smart Meter Simulator

```python
# Generates realistic consumption patterns based on:
# - Meter type (residential, commercial, industrial)
# - Time of day (hourly patterns)
# - Day of week (weekend vs weekday)
# - Random variation (10-20%)
# - Fraud injection (2% of meters report low values)

# Example hourly pattern for residential:
hourly_pattern = [
    0.4, 0.3, 0.3, 0.3, 0.4, 0.6,  # 0-5am  (low - sleeping)
    0.9, 1.2, 1.0, 0.7, 0.6, 0.7,  # 6-11am (morning rush)
    0.8, 0.7, 0.7, 0.8, 1.0, 1.4,  # 12-5pm (afternoon)
    1.6, 1.5, 1.3, 1.0, 0.7, 0.5,  # 6-11pm (evening peak)
]
```

#### Weather Impact on Renewables

```
┌───────────────────────────────────────────────────────────────┐
│                    WEATHER IMPACT FACTORS                     │
├───────────────────────────────────────────────────────────────┤
│                                                               │
│  SOLAR GENERATION:                                            │
│  ┌─────────────────┬────────────┐                             │
│  │ Cloud Condition │ Factor     │                             │
│  ├─────────────────┼────────────┤                             │
│  │ Clear           │ 1.0 (100%) │ ████████████████████        │
│  │ Partly Cloudy   │ 0.7 (70%)  │ ██████████████              │
│  │ Cloudy          │ 0.3 (30%)  │ ██████                      │
│  │ Overcast        │ 0.1 (10%)  │ ██                          │
│  │ Rainy           │ 0.05 (5%)  │ █                           │
│  └─────────────────┴────────────┘                             │
│                                                               │
│  WIND GENERATION:                                             │
│  ┌─────────────────┬────────────┐                             │
│  │ Wind Speed      │ Factor     │                             │
│  ├─────────────────┼────────────┤                             │
│  │ Calm (<3 m/s)   │ 0.0 (0%)   │                             │
│  │ Light (3-6)     │ 0.3 (30%)  │ ██████                      │
│  │ Moderate (6-12) │ 0.7 (70%)  │ ██████████████              │
│  │ Strong (12-25)  │ 1.0 (100%) │ ████████████████████        │
│  │ Extreme (>25)   │ 0.0 (0%)   │ (shutdown for safety)       │
│  └─────────────────┴────────────┘                             │
└───────────────────────────────────────────────────────────────┘
```

### Spark Configuration

```python
# Key Spark settings (config/spark_config.py)
SparkSession.builder
    .master("local[*]")           # Use all CPU cores
    .config("spark.driver.memory", "4g")
    .config("spark.executor.memory", "4g")
    .config("spark.sql.shuffle.partitions", "8")
    .config("spark.default.parallelism", "8")
```

---

## 8. Output & Results

### Sample Dashboard Output

```
╔══════════════════════════════════════════════════════════════════════════╗
║                           GRID BALANCE RESULTS                           ║
╠══════════════════════════════════════════════════════════════════════════╣
║                                                                          ║
║  District  │ Demand (MWh) │ Supply (MW) │ Net Load │ Status              ║
║  ──────────┼──────────────┼─────────────┼──────────┼───────────────────  ║
║  D000      │    12,543.2  │   15,000.0  │  +2,456  │ ✅ NORMAL           ║
║  D001      │     8,921.5  │    6,500.0  │  -2,421  │ ⚠️  CRITICAL         ║
║  D002      │    15,234.1  │   18,000.0  │  +2,765  │ ✅ NORMAL           ║
║  D003      │     5,432.8  │    5,800.0  │    +367  │ ✅ OK               ║
║  ...       │              │             │          │                     ║
╠══════════════════════════════════════════════════════════════════════════╣
║  BLACKOUT RISK SUMMARY                                                   ║
║  • Districts at Risk: 2                                                  ║
║  • Total Deficit: -4,892 MW                                              ║
║  • Critical Districts: D001, D007                                        ║
╚══════════════════════════════════════════════════════════════════════════╝

╔══════════════════════════════════════════════════════════════════════════╗
║                          FRAUD DETECTION SUMMARY                         ║
╠══════════════════════════════════════════════════════════════════════════╣
║  Meters Analyzed:        5,234                                           ║
║  Fraud Cases Detected:     112                                           ║
║  Fraud Rate:             2.14%                                           ║
║  Est. Stolen Energy:    45,231 kWh                                       ║
║  Est. Revenue Loss:    $5,427.78                                         ║
╚══════════════════════════════════════════════════════════════════════════╝

╔══════════════════════════════════════════════════════════════════════════╗
║                           CARBON FOOTPRINT                               ║
╠══════════════════════════════════════════════════════════════════════════╣
║  Total Emissions:      12,456.32 tons CO2                                ║
║  Emission Intensity:   0.234 tons/MWh                                    ║
║  Renewable %:          38.5%                                             ║
║  Target (40%):         ❌ Not Met                                         ║
╚══════════════════════════════════════════════════════════════════════════╝
```

### Generated Charts

| Chart | Description | File |
|-------|-------------|------|
| Demand by District | Bar chart of total kWh per district | `demand_by_district.png` |
| Demand by Type | Pie chart (residential/commercial/industrial) | `demand_by_type.png` |
| Supply by Type | Bar chart by plant type | `supply_by_type.png` |
| Capacity Utilization | Horizontal bar chart | `capacity_utilization.png` |
| Grid Balance | Balance status by district | `grid_balance.png` |
| Z-Score Distribution | Histogram with fraud thresholds | `fraud_zscore_dist.png` |
| Fraud Status | Bar chart of meter statuses | `fraud_status_dist.png` |
| Emissions by Type | Pie chart of CO2 sources | `emissions_by_type.png` |
| Hourly Emissions | Line chart over 24 hours | `hourly_emissions.png` |
| Renewable Mix | Pie chart renewable vs fossil | `renewable_mix.png` |

---

## 9. Configuration

### Grid Parameters (`config/grid_parameters.py`)

```python
# Adjust these to change simulation scale
NUM_DISTRICTS = 10              # Number of geographic regions
SIMULATION_DAYS = 3             # Days of data to generate

# Meters per district (by type)
DISTRICT_TYPES = {
    "industrial":  {"meters_range": (100, 300)},
    "commercial":  {"meters_range": (200, 500)},
    "residential": {"meters_range": (500, 1000)},
    "mixed":       {"meters_range": (300, 600)},
}

# Power plant capacities (MW)
PLANT_BASE_CAPACITY = {
    "nuclear": 1000,
    "coal": 600,
    "gas": 400,
    "hydro": 300,
    "wind": 150,
    "solar": 100,
}
```

### Thresholds (`config/thresholds.py`)

```python
# Grid stability
CRITICAL_DEFICIT = -0.15   # Below -15% = blackout risk
WARNING_DEFICIT = -0.05    # Below -5% = warning

# Fraud detection
FRAUD_ZSCORE_CRITICAL = 3.0   # |Z| > 3 = likely fraud
FRAUD_ZSCORE_WARNING = 2.5    # |Z| > 2.5 = suspicious

# Carbon targets
RENEWABLE_TARGET = 0.40    # 40% renewable goal
EMISSION_TARGET = 0.2      # tons CO2/MWh target
```

---

## 10. Troubleshooting

### Common Issues

#### Java Version Error

```
Error: Java 25 detected. Apache Spark requires Java 11 or 17.
```

**Solution:**
```bash
# Install Java 17
brew install openjdk@17  # macOS

# The run.sh script auto-detects Java 17 from Homebrew
./run.sh
```

#### Memory Error

```
java.lang.OutOfMemoryError: Java heap space
```

**Solution:** Edit `config/spark_config.py`:
```python
.config("spark.driver.memory", "8g")  # Increase from 4g
```

#### Data Not Found

```
Error: Data not found at data/raw/meter_readings.csv
```

**Solution:**
```bash
# Regenerate data
./run.sh --regenerate
```

#### Spark Stage Stuck

If processing seems stuck at `[Stage X: (0 + 8) / 8]`:

1. This is normal for large datasets
2. Wait - it's processing in parallel
3. Check `data/raw/meter_readings.csv` size
4. Reduce data size in `config/grid_parameters.py`

### Performance Tips

1. **Reduce data size** for faster testing:
   ```python
   # In grid_parameters.py
   SIMULATION_DAYS = 1
   NUM_DISTRICTS = 5
   ```

2. **Install psutil** to eliminate warnings:
   ```bash
   pip install psutil
   ```

3. **Use SSD** - Spark writes shuffle data to disk

4. **Close other applications** - Spark uses all CPU cores

---

## License

MIT License - Free for educational and commercial use.

---

## Acknowledgments

- Apache Spark team for the distributed computing framework
- Faker library for realistic data generation
- Rich library for beautiful console output

---


```
   ___                        ___      _     _     _    ___
  / _ \_____      _____ _ __ / _ \_ __(_) __| |   / \  |_ _|
 / /_)/ _ \ \ /\ / / _ \ '__/ /_\/ '__| |/ _` |  / _ \  | |
/ ___/ (_) \ V  V /  __/ | / /_\\| |  | | (_| | / ___ \ | |
\/    \___/ \_/\_/ \___|_| \____/|_|  |_|\__,_|/_/   \_\___|

       Distributed Energy Intelligence Platform
```
